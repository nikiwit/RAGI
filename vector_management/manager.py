...
class VectorStoreManager:
    ...
    def check_vector_store_health(vector_store):
        ...
        try:
            ...
            query_success = False
            try:
                test_results = vector_store.similarity_search("test query university", k=1)
                if test_results:
    ...
    @staticmethod
    def load_embeddings_backup(embeddings, filepath=None, collection_name="ragi_collection"):
        """
    ...
    def get_or_create_vector_store(cls, chunks=None, embeddings=None, persist_directory=None):
        ...
        collection_name = "ragi_collection"
    ...
    def print_document_statistics(vector_store):
        ...
        try:
            ...
            # Count documents by filename and folder
            folder_counts = {}
            project_counts = {}
            ...
            for metadata in all_metadata:
                ...
                # Count by folder
                if metadata:
                    folder_name = metadata.get('folder_name', 'root')
                    folder_counts[folder_name] = folder_counts.get(folder_name, 0) + 1

                    project = metadata.get('project', 'root')
                    project_counts[project] = project_counts.get(project, 0) + 1

                # Count knowledge base specific pages
                if metadata.get('content_type') == 'kb_page':
                    kb_count += 1
            ...
            # Print knowledge base specific statistics
            if kb_count > 0:
                print(f"\nKnowledge Base: {kb_count} pages, including {faq_count} FAQs")

            # Print folder statistics
            if len(project_counts) > 1 or (len(project_counts) == 1 and 'root' not in project_counts):
                print(f"\n📂 Documents by Project:")
                for project, count in sorted(project_counts.items(), key=lambda x: x[1], reverse=True):
                    print(f"  - {project}: {count} chunks")

            if len(folder_counts) > 1 or (len(folder_counts) == 1 and 'root' not in folder_counts):
                print(f"\n📁 Documents by Folder:")
