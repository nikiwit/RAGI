...
# Import RAGI system components
from app import RAGI
...
class APIState:
    def __init__(self):
        self._ragi_instance = None
        self._ragi_lock = threading.Lock()
        self._processing_status = {}
        self._status_lock = threading.Lock()

    def get_ragi_instance(self):
        """Get or create RAGI instance (thread-safe singleton)."""
        if self._ragi_instance is None:
            with self._ragi_lock:
                if self._ragi_instance is None:
                    logger.info("Initializing RAGI system...")
                    self._ragi_instance = RAGI()
                    if not self._ragi_instance.initialize():
                        logger.error("Failed to initialize RAGI system")
                        raise RuntimeError("Failed to initialize RAGI system")
                    logger.info("RAGI system initialized successfully")
        return self._ragi_instance
...
# Initialize RAGI system at startup
try:
    logger.info("Pre-initializing RAGI system at startup...")
    api_state.get_ragi_instance()
    logger.info("RAGI system pre-initialization completed")
except Exception as e:
    logger.error(f"Failed to pre-initialize RAGI system: {e}")
...
def setup_session_memory(ragi, session):
    """Update RAGI's memory references to a session."""
    ragi.memory = session.memory
    for handler in [ragi.conversation_handler, ragi.retrieval_handler, ragi.query_router]:
        if handler and hasattr(handler, 'memory'):
            handler.memory = ragi.memory

def handle_session_management(ragi, session_id, device_id):
    """Handle session creation and loading logic."""
    is_new_session = False

    if session_id:
        existing_session = ragi.session_manager.load_session(session_id)
        if not existing_session:
            ...
            session = ragi.session_manager.create_session(session_title)
            is_new_session = True
            session_id = session.metadata.session_id
        else:
            setup_session_memory(ragi, existing_session)
    else:
        ...
        is_new_session = True
        session_id = session.metadata.session_id
        setup_session_memory(ragi, session)
...
def with_api_security(f):
    ...
    def decorated_function(*args, **kwargs):
        try:
            ragi = api_state.get_ragi_instance()
            if hasattr(ragi, 'command_handler') and ragi.command_handler:
                ragi.command_handler.set_api_mode(True)
...
def chat_query():
    ...
    ragi = api_state.get_ragi_instance()
    session_id, is_new_session = handle_session_management(ragi, session_id, device_id)
    
    # Process query
    query_analysis = ragi.input_processor.analyze_query(query)
    ...
    if wants_stream:
        def generate_response_stream():
            ...
            response_stream, _ = ragi.query_router.route_query(query_analysis, stream=True)
            full_response = ""
            ...
            if full_response and full_response.strip():
                ragi.session_manager.add_conversation(query, full_response.strip())
    ...
    response, _ = ragi.query_router.route_query(query_analysis, stream=False)
    response = handle_response_format(response)
    api_state.clear_processing_status(session_id)
    
    # Save conversation
    if response and response.strip():
        ragi.session_manager.add_conversation(query, response.strip())
...
def chat_conversation():
    ...
    if wants_stream:
        def generate_conversation_stream():
            ...
            response_stream = ragi.conversation_handler.handle_conversation(query, stream=True)
            full_response = ""
    ...
    # Non-streaming response
    response = ragi.conversation_handler.handle_conversation(query, stream=False)
    response = handle_response_format(response)
    api_state.clear_processing_status(session_id)
...
def chat_stream():
    ...
    def generate_stream():
        try:
            ragi = api_state.get_ragi_instance()
            session_id_final, is_new_session = handle_session_management(ragi, session_id, device_id)
...
def chat_history(session_id):
    ...
    ragi = api_state.get_ragi_instance()
    session = ragi.session_manager.load_session(session_id)
...
def list_sessions():
    ...
    ragi = api_state.get_ragi_instance()
    sessions_data = ragi.session_manager.list_sessions()
    ...
    current_session = ragi.session_manager.current_session
...
def create_session():
    ...
    ragi = api_state.get_ragi_instance()
    session = ragi.session_manager.create_session(title)
    setup_session_memory(ragi, session)
...
def clear_session(session_id):
    ...
    if current_session and current_session.metadata.session_id == session_id:
        ragi.session_manager.clear_current_session()
    ...
    ragi.session_manager.save_session(session)
