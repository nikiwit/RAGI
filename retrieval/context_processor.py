...
class ContextProcessor:
    ...
    def _score_documents(self, documents: List[Document], keywords: List[str], query_type: QueryType) -> List[Tuple[Document, float, DocumentRelevance]]:
        """
        Score documents based on relevance to the query, with knowledge base optimizations.
        ...
        for doc in documents:
            ...
            is_apu_kb = doc.metadata.get('content_type') == 'kb_page'
            ...
            # Boost for knowledge base pages
            if is_apu_kb:
    ...
    def _format_documents(self, selected_docs: List[Tuple[Document, DocumentRelevance]], query_analysis: Dict[str, Any]) -> str:
        ...
        for doc, relevance in selected_docs:
            ...
            # Track unique page titles for knowledge base pages
        ...
        if title_count > 0:
            summary_header += f" from {title_count} topics in the knowledge base"
        
        if query_type == QueryType.ACADEMIC:
            summary_header = f"Found {doc_count} relevant sections about academic procedures "
        elif query_type == QueryType.ADMINISTRATIVE:
            summary_header = f"Found {doc_count} relevant sections about administrative processes "
        elif query_type == QueryType.FINANCIAL:
            summary_header = f"Found {doc_count} relevant sections about financial matters "
