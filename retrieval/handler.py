...
from ragi_types import QueryType, RetrievalStrategy
from .system_info import SystemInformation
# FAQ matching removed - using pure semantic + hybrid search for all documents
...
class RetrievalHandler:
    ...
    def __init__(self, vector_store, embeddings, memory, context_processor):
        ...
        # Folder filter for metadata pre-filtering
    ...
    def set_folder_filter(self, folder_filter: Dict[str, Any] = None):
        """
        Set metadata filter for folder-based retrieval.

        Args:
            folder_filter: Dictionary with folder metadata filters
                Examples:
                - {"folder_name": "technical-docs"}
                - {"project": "project-alpha"}
                - {"folder_path": "project-alpha/docs"}
        """
        self.folder_filter = folder_filter
        if folder_filter:
            logger.info(f"Applied folder filter: {folder_filter}")
        else:
            logger.info("Cleared folder filter")

    def clear_folder_filter(self):
        """Clear any active folder filter."""
        self.folder_filter = None

    def get_available_folders(self) -> Dict[str, Any]:
        """
        Get list of all available folders in the vector store.
        ...
        try:
            ...
            for metadata in metadatas:
                if metadata:
                    ...
                    folder_paths[folder_path] += 1

            return {
                'projects': dict(projects.most_common()),
                'folders': dict(folders.most_common()),
                'folder_paths': dict(folder_paths.most_common()),
                'total_documents': len(metadatas)
            }

        except Exception as e:
            logger.error(f"Error getting folder information: {e}")
            return {}

    def _matches_folder_filter(self, metadata: Dict[str, Any]) -> bool:
        """
        Check if document metadata matches the active folder filter.

        Args:
            metadata: Document metadata dictionary

        Returns:
            bool: True if metadata matches filter (or no filter active)
        """
        if not self.folder_filter or not metadata:
            return True

        # Check each filter condition
        for key, value in self.folder_filter.items():
    ...
    def _get_empty_response_fallback(self, query: str) -> str:
        ...
        return (
            ...
            "Could you please try rephrasing your question? I'm here to help with information from the knowledge base information "
    ...
    def _get_short_response_fallback(self, query: str, original_response: str) -> str:
        ...
        return (
            ...
            "your documents, procedures, and policies."
        )
    ...
    def _get_error_response_fallback(self, query: str) -> str:
        ...
        return (
            ...
            "or feel free to rephrase your question. I'm here to help with information from your knowledge base including "
    ...
    def process_query(self, query_analysis: Dict[str, Any], stream=False) -> Union[str, Iterator[str]]:
        ...
        if not self._validate_input_sanity(original_query):
            ...
            nonsense_response = (
                ...
                "I'm here to help with information from the knowledge base information like academic procedures, "
        ...
        # FAQ matching removed - proceed directly with standard retrieval
    ...
    # FAQ matching methods removed - using pure semantic + hybrid search for all documents
    ...
    def _keyword_retrieval(self, query: str) -> List[Document]:
        """
        Perform keyword-based retrieval, enhanced for the knowledge base.
        ...
        # Apply folder filtering if set
        if self.folder_filter:
            filtered_docs = []
            filtered_metadatas = []
            filtered_ids = []
            for i, metadata in enumerate(metadatas):
                if self._matches_folder_filter(metadata):
            ...
            ids = filtered_ids
            logger.info(f"Folder filter reduced documents from {len(all_docs.get('documents', []))} to {len(documents)}")
        ...
        for i, doc_text in enumerate(documents):
            ...
            # Check if this is an knowledge base page
            is_apu_kb = metadata.get('content_type') == 'kb_page'
    ...
    def _rerank_documents(self, docs: List[Document], query: str) -> List[Document]:
        ...
        for doc in docs:
            ...
            # Check if document is a knowledge base page
            kb_boost = 0
            if doc.metadata.get('content_type') == 'kb_page':
                kb_boost = 0.2

            # Calculate final score
            final_score = base_score + phrase_match + keyword_match + kb_boost
    ...
    def _calculate_confidence_score(self, docs: List[Document], query: str) -> float:
        ...
        # Factor 3: Metadata quality (knowledge base pages are more reliable)
        metadata_score = 0
        for doc in docs:
            if doc.metadata.get('content_type') == 'kb_page':
                metadata_score += 0.3
    ...
    def _get_boundary_response(self, query: str) -> str:
        ...
        if any(word in query_lower for word in ['program', 'course', 'degree', 'admission', 'requirement']):
            return (
                "I don't have detailed information about programs and admission requirements. "
                "For the most current information about programs, courses, and admission criteria, "
                "please contact the Admissions Office or check the official website."
            )
            ...
            return (
                ...
                "please log in to the system or contact the Registrar's Office."
            )

        elif any(word in query_lower for word in ['weather', 'temperature', 'climate', 'rain', 'sunny']):
            return (
                "I'm designed to help with information from the knowledge base rather than weather updates. "
                "For weather information, please check your local weather app or website. "
                "Is there anything about services or procedures I can help you with instead?"
            )
            ...
            return (
                ...
                "However, I can help you with schedules, timetables, and operating hours. "
                "What timing information from the knowledge base do you need?"
            )

        elif any(word in query_lower for word in ['news', 'current events', 'politics', 'sports']):
            return (
                "I focus on information from the knowledge base rather than general news. "
                "For current events, please check news websites or apps. "
                "I'd be happy to help with announcements, procedures, or services instead!"
            )
            ...
            return (
                ...
                "For faculty contact information and office hours, please check the official website "
            ...
            return (
                ...
                "For information about housing and accommodation, please contact Student Services."
            )

        else:
            return (
                "I don't have specific information about that topic in my knowledge base. "
                "I'm designed to help with information from the knowledge base. "
    ...
    def _create_prompt(self, input_dict: Dict[str, Any]) -> str:
        ...
        # Prompt creation based on confidence score
        if confidence_score > 0.65:
            prompt = f"""You are RAGI, an AI knowledge assistant. Answer this user's question directly and naturally.
...
- If the question contains harmful content (suicide, self-harm, etc.), respond: "I'm designed to help with information from the knowledge base. If you're experiencing distress, please reach out to counseling services or contact emergency services. Is there something from the knowledge base I can help with instead?"
- If the question is clearly outside the knowledge base scope (weather, sports, general news, entertainment, etc.), respond: "I don't have information about that topic. I'm designed to help with information from the knowledge base. Is there something I can help you with instead?"
...
Answer:"""
            ...
            prompt = f"""You are RAGI, an AI knowledge assistant. The user asked a question, but the available information may only be partially related.
    ...
    2. **SAFETY CHECK**: If the question contains harmful content (suicide, self-harm, etc.), respond: "I'm designed to help with information from the knowledge base. If you're experiencing distress, please reach out to counseling services or contact emergency services. Is there something from the knowledge base I can help with instead?"
    3. **SCOPE CHECK**: If the question is clearly outside the knowledge base scope (weather, sports, general news, entertainment, etc.), respond: "I don't have information about that topic. I'm designed to help with information from the knowledge base. Is there something I can help you with instead?"
    ...
    6. If the available information is completely unrelated, say: "I don't have information about that topic. I'm designed to help with information from the knowledge base. Is there something I can help you with instead?"
    ...
    Answer:"""
            ...
            prompt = f"""You are RAGI, an AI knowledge assistant. Answer the question using the information available.
...
3. **CRITICAL**: Preserve ALL URLs and links exactly as they appear.
...
8. **REQUIRED**: When the source describes specific situations (internships, attendance issues, etc.), present them as conditional options: "If you are doing an internship...", "For users who...", "In cases where..."
9. NEVER personalize generic information (e.g., don't say "your attendance is 73%" - say "if attendance is below 80%").
10. NEVER assume specific personal details about the user (attendance, fees, grades, etc.).
11. Provide general guidance that covers different scenarios without assuming which applies to the user.
12. **UX CRITICAL**: If the available information is completely unrelated or irrelevant to the question, say "I don't have information about that topic" - NEVER mention "provided information", "documents", "sections", "knowledge base", or "context". However, if there is ANY relevant information that helps answer the question, even partially, you MUST provide it rather than saying you don't have information.
...
14. Use a helpful and professional tone appropriate for an assistant.

Answer:"""

        return prompt
    
    # FAQ validation method removed - not needed with pure semantic + hybrid search
